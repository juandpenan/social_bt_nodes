  ...
  phase_(Phase::IDLE)
{
  ...
  if (!node_any) {
    throw BT::RuntimeError("Ask: 'node' not found in blackboard");
  }
  node_ = node_any;
}

BT::NodeStatus Ask::onStart()
{
  ...
  if (!tts_client_->wait_for_service(std::chrono::milliseconds(timeout_ms_))) {
    RCLCPP_WARN(
      node_->get_logger(),
      "Ask: TTS service '%s' not available yet", tts_service_name_.c_str());
  ...
  RCLCPP_INFO(node_->get_logger(), "Ask: Asking question '%s'", question_.c_str());
...
BT::NodeStatus Ask::onRunning()
{
  if (phase_ == Phase::WAITING_TTS) {
    ...
    if (!tts_result->success) {
      RCLCPP_ERROR(node_->get_logger(), "Ask: TTS failed: %s", tts_result->debug.c_str());
    ...
    if (!stt_client_->wait_for_service(std::chrono::milliseconds(timeout_ms_))) {
      RCLCPP_WARN(
        node_->get_logger(),
        "Ask: STT service '%s' not available yet", stt_service_name_.c_str());
  ...
  if (phase_ == Phase::WAITING_STT) {
    ...
    if (!stt_result->success) {
      RCLCPP_ERROR(node_->get_logger(), "Ask: STT failed: %s", stt_result->message.c_str());
      return bt_failure(config(), registrationName(), "failed to transcribe: " + stt_result->message);
    }

    setOutput("answer", stt_result->message);
    RCLCPP_INFO(node_->get_logger(), "Ask: Captured answer '%s'", stt_result->message.c_str());
    phase_ = Phase::IDLE;
    return BT::NodeStatus::SUCCESS;
  }

  return bt_failure(config(), registrationName(), "invalid internal state in Ask");
}

void Ask::onHalted()
{
  RCLCPP_WARN(node_->get_logger(), "Ask: Halted");
