  ...
  waiting_for_speech_completion_(false)
{
  ...
  if (!node_any) {
    throw BT::RuntimeError("SpeakReactive: 'node' not found in blackboard");
  }
  node_ = node_any;
}

BT::NodeStatus SpeakReactive::onStart()
{
  // Read input 
  if (!getInput("text", text_)) {
    ...
    if (input_it != config().input_ports.end()) { 
      ...
      RCLCPP_ERROR(node_->get_logger(), "SpeakReactive: missing required input 'text'");
  ...
  if (resolve_blackboard_template(text_, config().blackboard, resolved_text, resolve_error)) {
    ...
    if (!resolve_error.empty() && resolve_error.find("no blackboard placeholders") == std::string::npos) {
      RCLCPP_WARN(node_->get_logger(), "SpeakReactive: template resolution warning: %s", resolve_error.c_str());
    }
  }
  ...
  if (!client_->wait_for_service(std::chrono::milliseconds(timeout_ms_))) {
    RCLCPP_WARN(node_->get_logger(), 
      "SpeakReactive: Service '%s' not available yet", service_name_.c_str());
  ...
  RCLCPP_INFO(node_->get_logger(), "SpeakReactive: Speaking '%s'", text_.c_str());
...
BT::NodeStatus SpeakReactive::onRunning()
{
    ...
    if (status == std::future_status::ready) {
      ...
      if (!result->success) {
        RCLCPP_ERROR(node_->get_logger(), 
          "SpeakReactive: Speech failed: %s", result->debug.c_str());
      ...
      RCLCPP_INFO(node_->get_logger(), 
        "SpeakReactive: TTS service responded, waiting %d ms for speech completion", 
        duration_ms);
    }
  ...
  if (elapsed >= speech_duration_) {
    RCLCPP_INFO(node_->get_logger(), "SpeakReactive: Speech completed");
...
void SpeakReactive::onHalted()
{
  RCLCPP_WARN(node_->get_logger(), "SpeakReactive: Halted");
